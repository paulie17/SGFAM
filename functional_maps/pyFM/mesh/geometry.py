...
def grad_mat(vertices, faces, normals=None, face_areas=None, order_style='C'):
    ...
    Joseph: idk why they wrote it as 3*m, but it should be m*3 since the flatten is in fortran order
    """
...
def grad_f(f, vertices, faces, normals, face_areas=None, use_sym=False, grads=None):
    ...
    if not use_sym:
        ...
        if grads is None:
            # Joseph: the resulting vector is a line with the magnitude of 1 / ((2*face_area)/(v3-v2)) , which is the inverse of the length of the line from v1 perpenduclar to e23
            # the vector's direction is just pointing from v1 to e23
            # so basically just barycentric coordinate(since perpendicular line from any point to e23 is basically just barycentric coordinate)
            grad1 = np.cross(normals, v3-v2)/(2*face_areas[:,None])  # (m,3)
            grad2 = np.cross(normals, v1-v3)/(2*face_areas[:,None])  # (m,3)
            grad3 = np.cross(normals, v2-v1)/(2*face_areas[:,None])  # (m,3)
...
def farthest_point_sampling_call_sub(d_func, k, sub_points, return_sub_inds=False, random_init=True, verbose=False):
    """
